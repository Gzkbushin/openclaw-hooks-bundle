...
- [ ] Fix plugins/context-mode/index.ts: remove `createRequire` from `node:module`, switch all `api.registerHook({event, handler})` to `api.on(event, handler, {priority})` positional form, do NOT modify functional logic (~800 lines), ONLY change plugin wrapper and hook registration calls, export plain object (NOT definePluginEntry)
- [ ] Fix plugins/context-mode/package.json: change `name` from `"context-mode-openclaw"` to `"context-mode"`
